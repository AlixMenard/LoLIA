...
class Tournament():
    def __init__(self, leagueId, tournamentId, tournamentData=[]):
        ...
        queryData = downloadMeta('getSchedule', {'tournamentId': self.id})
...
class Event():
    ...
    def getGame(self, gameId, **kwargs):
        gameData = {
            'winner': [team['name'] for team in self.data['match']['teams'] if team['result']['gameWins'] == 1],
...
class Game():
    ...
    def get_window(self, starting_time=get_latest_date()):
        ...
        response = session.get(
            LIVESTATS_API + f'/window/{self.id}',
            params={
    ...
    def get_details(self, starting_time=get_latest_date(), participant_ids=None):
        API_KEY = {'x-api-key': '0TvQnueqKa5mxJntVWt0w4LpLfEkrV1Ta8rQBb9Z'}
        LIVESTATS_API = 'https://feed.lolesports.com/livestats/v1'
        session = requests.Session()
        session.headers.update(API_KEY)
        response = session.get(
            LIVESTATS_API + f'/details/{self.id}',
            params={
                'startingTime': starting_time,
        ...
        try:
            ...
            if (not _os.path.isfile(fname)) or (overwrite):
                ...
                with open(fname, 'w') as fp:
                    _json.dump(gameData, fp, indent=4)
                if verbose:
